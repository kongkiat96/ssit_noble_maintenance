    ...
    if (isset($_POST["submit"]) && $_POST['submit'] == "Submit") {
        ...
        switch ($test_type) {
            case 'smtp':
                ...
                $example_code .= "\n\$mail->isSMTP();";
                $example_code .= "\n\$mail->SMTPDebug  = " . (integer) $smtp_debug . ";";
                $example_code .= "\n\$mail->Host       = \"" . htmlspecialchars($smtp_server) . "\";";
                $example_code .= "\n\$mail->Port       = \"" . htmlspecialchars($smtp_port) . "\";";
                $example_code .= "\n\$mail->SMTPSecure = \"" . htmlspecialchars(strtolower($smtp_secure)) . "\";";
                $example_code .= "\n\$mail->SMTPAuth   = " . (array_key_exists(
                    'smtp_authenticate',
                    $_POST
                ) ? 'true' : 'false') . ";";
                if (array_key_exists('smtp_authenticate', $_POST)) {
                    $example_code .= "\n\$mail->Username   = \"" . htmlspecialchars($authenticate_username) . "\";";
                    $example_code .= "\n\$mail->Password   = \"" . htmlspecialchars($authenticate_password) . "\";";
                }
                break;
            case 'mail':
        ...
        try {
            if ($_POST['From_Name'] != '') {
                ...
                $example_code .= "\n\$mail->addReplyTo(\"" .
                    htmlspecialchars($from_email) . "\", \"" . htmlspecialchars($from_name) . "\");";
                $example_code .= "\n\$mail->setFrom(\"" .
                    htmlspecialchars($from_email) . "\", \"" . htmlspecialchars($from_name) . "\");";
            } else {
                ...
                $example_code .= "\n\$mail->addReplyTo(\"" . htmlspecialchars($from_email) . "\");";
                $example_code .= "\n\$mail->setFrom(\"" .
                    htmlspecialchars($from_email) . "\", \"" . htmlspecialchars($from_email) . "\");";
            }

            if ($_POST['To_Name'] != '') {
                $mail->addAddress($to, $to_name);
                $example_code .= "\n\$mail->addAddress(\"$to\", \"" . htmlspecialchars($to_name) . "\");";
            } else {
                $mail->addAddress($to);
                $example_code .= "\n\$mail->addAddress(\"$to\");";
            }
            ...
            if ($_POST['bcc_Email'] != '') {
                ...
                foreach ($indiBCC as $key => $value) {
                    $mail->addBCC($value);
                    $example_code .= "\n\$mail->addBCC(\"" . htmlspecialchars($value) . "\");";
                }
            }
            ...
            if ($_POST['cc_Email'] != '') {
                ...
                foreach ($indiCC as $key => $value) {
                    $mail->addCC($value);
                    $example_code .= "\n\$mail->addCC(\"" . htmlspecialchars($value) . "\");";
                }
            }
        ...
        $example_code .= "\n\$mail->Subject  = \"" . htmlspecialchars($subject) .
            ' (PHPMailer test using ' . htmlspecialchars(strtoupper($test_type)) . ')";';
        ...
        $example_code .= "\n  \$results_messages[] = \"Message has been sent using " .
            htmlspecialchars(strtoupper($test_type)) . "\";";
        $example_code .= "\n}";
